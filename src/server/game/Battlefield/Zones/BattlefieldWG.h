...
const BfWGTurretData AttackTurret[3]={
	//Tower west
	{
		190356,
		2,
...
struct BfWGGameObjectBuilding
{
	...
	CreatureSet m_TurretBottomList;
	CreatureSet m_TurretTopList;
    ...
    void Init(GameObject* go,uint32 type,uint32 worldstate,uint32 nameid)
    {
	    ...
	    if(towerid>=0)
	    {
			...
			//Spawn Turret Attack bottom
			for(uint8 i = 0; i < AttackTurret[towerid].nbTurretBottom; i++)
			{
				BfWGTurretPos turretpos = AttackTurret[towerid].TurretBottom[i];
				if (Creature* turret = m_WG->SpawnCreature(28366, turretpos.x, turretpos.y, turretpos.z, turretpos.o, 0))
				{
					m_TurretBottomList.insert(turret);
					turret->setFaction(WintergraspFaction[m_WG->GetAttackerTeam()]);
					turret->SetVisibility(VISIBILITY_OFF);
				}
			}
			//Spawn Turret Attack top
			for(uint8 i = 0; i < AttackTurret[towerid].nbTurretTop; i++)
			{
				BfWGTurretPos turretpos = AttackTurret[towerid].TurretTop[i];
				if (Creature* turret = m_WG->SpawnCreature(28366, turretpos.x, turretpos.y, turretpos.z, turretpos.o, 0))
				{
					m_TurretTopList.insert(turret);
					turret->setFaction(WintergraspFaction[m_WG->GetAttackerTeam()]);
					turret->SetVisibility(VISIBILITY_OFF);
				}
			}
	...
	{
		for (CreatureSet::const_iterator itr = m_TurretBottomList.begin(); itr != m_TurretBottomList.end(); ++itr)
		{
			if (disable)
			{
				(*itr)->setFaction(WintergraspFaction[m_WG->GetAttackerTeam()]);
		...
		for (CreatureSet::const_iterator itr = m_TurretTopList.begin(); itr != m_TurretTopList.end(); ++itr)
		{
			if (disable)
			{
				(*itr)->setFaction(WintergraspFaction[m_WG->GetAttackerTeam()]);

...
bool BattlefieldWG::SetupBattlefield()
{
    ...
    if(( sWorld.getWorldState(3801) == 0 ) && (sWorld.getWorldState(3802) == 0) && ( sWorld.getWorldState(ClockWorldState[0]) == 0 ) )
	{
	    ...
	    sWorld.setWorldState(3802,TEAM_ALLIANCE);
...
void BattlefieldWG::HandleKill(Player *killer, Unit *victim)
{
    ...
    if(IsWarTime())
    {
	   ...
	   if (victim->GetTypeId() == TYPEID_PLAYER)
	    ...
	    for (CreatureSet::const_iterator itr = m_vehicles[killer->GetTeamId() ? TEAM_ALLIANCE : TEAM_HORDE].begin(); itr != m_vehicles[killer->GetTeamId() ? TEAM_ALLIANCE : TEAM_HORDE].end(); ++itr)
		{
			if ((*itr) && (victim->ToCreature()->GetEntry() == (*itr)->GetEntry()) && !again)
			{
			    again = true;
			    for (GuidSet::const_iterator p_itr = m_PlayersInWar[killer->GetTeamId()].begin(); p_itr != m_PlayersInWar[killer->GetTeamId()].end(); ++p_itr)					
				{
		...
		for(CreatureSet::const_iterator itr = KeepCreature[killer->GetTeamId() ? TEAM_ALLIANCE : TEAM_HORDE].begin(); itr != KeepCreature[killer->GetTeamId() ? TEAM_ALLIANCE : TEAM_HORDE].end(); ++itr)
			{
				if ((*itr) && (victim->ToCreature()->GetEntry() == (*itr)->GetEntry()) && !again)
				{
					again = true;

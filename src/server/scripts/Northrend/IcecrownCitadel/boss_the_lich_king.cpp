...
class boss_the_lich_king : public CreatureScript
{
    public:
        ...
        {
            boss_the_lich_kingAI(Creature* pCreature) : BossAI(pCreature, DATA_THE_LICH_KING), summons(me)
            {
                ASSERT(instance);
                pLichKing = me;
            }
            ...
            void UpdateAI(const uint32 uiDiff)
            {
                ...
                if(HealthBelowPct(12))
                {
                    if(!TriggerSpawned)
                    {
                        pSafeZone = me->SummonCreature(NPC_TRIGGER, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_CORPSE_DESPAWN, 360000);
                ...
                if(bEnding)
                {
                    if (m_uiEndingTimer <= uiDiff)
                    {
                        ...
                        case 18:
                            ...
                            break;
                        case 19:
                            pFather = me->SummonCreature(NPC_MENETHIL, me->GetPositionX()+5, me->GetPositionY()+5, me->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 9999999);
    ...
    public:
        ...
        struct npc_tirion_iccAI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if(instance->GetData(DATA_THE_LICH_KING) != NOT_STARTED)
                {
